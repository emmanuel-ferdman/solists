...
function testEntries(dsClass) {
  describe(`${dsClass.name} Base: Test the "entries" method`, () => {
    it('should check "entries" on []', () => {
      const array = [];
    ...
    it('should check "entries" on [1,2,3]', () => {
      const array = [1, 2, 3];
      const ds = new dsClass(array);
      assert.equal(ds.length, array.length);
      assert(ds.isEqual(array));
    ...
    it('should check "entries" on [null,undefined,NaN,{},Infinity,"string",-0,1.5,true,new Date("21 Dec 1997 14:12:00 UTC")]', () => {
      const array = [
        null,
        undefined,
        NaN,
